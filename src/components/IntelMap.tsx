...
function iconForEvent(type: string, heading?: number) {
  if (type === "aircraft" || type === "airport") {
    // Plane icon: rotated to actual heading. Leaflet rotates via CSS on the wrapper.
...
const createIntelIcon = (event: IntelligenceEvent, selected: boolean) => {
  ...
  // Heading: aircraft use real track; vessels use heading; default 0 (north/up)
  const heading = event.heading ?? 0;
  // Rotate inner icon for aircraft/vessel so it faces true heading
  const shouldRotate = isAircraft || isVessel;
  ...
  return L.divIcon({
    html: `
      <div class="intel-marker ${selected ? "is-selected" : ""} ${isMilitary ? "is-military" : ""}" style="--marker-color:${color};${militaryRing}">
        <span style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;transform:${innerRotate};transition:transform 0.3s;">
          ${renderToString(iconForEvent(event.type, heading))}
        </span>
      </div>
    `,
...
function DraggablePanel({ title, onClose, children, defaultPos, defaultSize }: {
  ...
  const toggleMaximize = () => {
    if (maximized) {
      setPos(prevState.pos); setSize(prevState.size); setMaximized(false);
    } else {
...
// Rich popup content depending on event type
function PopupContent({ event, onAnalyze }: { event: IntelligenceEvent; onAnalyze?: () => void }) {
  ...
  return (
    <div className="popup-card popup-card--rich">
      ...
      {isSatellite && (
        <div className="popup-grid">
          <div><span>Altitude</span><strong>{altStr !== "—" ? altStr : (event.details?.match(/Alt: (\d+) km/)?.[1] ?? "—") + " km"}</strong></div>
...
export default function IntelMap({ apiBase, events, selectedEvent, onEventClick, onAnalyzeEvent, region, showBorders }: IntelMapProps) {
  ...
  return (
    <div className="relative w-full h-full bg-[#0b1118]">
        ...
        <DraggablePanel title={`NASA GIBS imagery — ${region.label}`} onClose={() => setShowGibsPanel(false)} defaultPos={{ x: 64, y: 88 }} defaultSize={{ w: 620, h: 460 }}>
          <div className="panel-toolbar">
            {GIBS_LAYERS.map((layer, i) => (
              <button key={layer.id} onClick={() => setGibsLayer(i)} className={gibsLayer === i ? "segmented is-active" : "segmented"}>{layer.name}</button>
            ))}
      ...
      {showJammingPanel && (
        <DraggablePanel title="GPS jamming monitor — gpsjam.org" onClose={() => setShowJammingPanel(false)} defaultPos={{ x: 560, y: 88 }} defaultSize={{ w: 520, h: 420 }}>
          <iframe src={`https://gpsjam.org/?lat=${tileCenter}&z=${region.zoom + 1}`} className="w-full h-full border-0" title="GPS jamming" />
        </DraggablePanel>
      )}

      {showSatellitePanel && (
        <DraggablePanel title="Satellite tracker — N2YO live passes" onClose={() => setShowSatellitePanel(false)} defaultPos={{ x: 300, y: 110 }} defaultSize={{ w: 560, h: 460 }}>
          <div className="panel-toolbar">
            <button className="segmented is-active">ISS passes</button>
            <button className="segmented" onClick={() => {}}>All sats</button>
          </div>
          <iframe
          ...
          return (
            <React.Fragment key={event.id}>
              <Marker
                ...
                icon={createIntelIcon(event, selected)}
      ...
      <div className="map-toolbar">
        ...
        <button onClick={() => setShowVesselLayer(!showVesselLayer)} className={showVesselLayer ? "tool-toggle is-active" : "tool-toggle"} title="AIS vessel overlay">
          <Ship className="w-4 h-4" /> AIS
        </button>
        <button onClick={() => setShowFlightLayer(!showFlightLayer)} className={showFlightLayer ? "tool-toggle is-active" : "tool-toggle"} title="ADS-B flight overlay">
          <Plane className="w-4 h-4" /> ADS-B
        </button>
        <button onClick={() => setShowSatellitePanel(!showSatellitePanel)} className={showSatellitePanel ? "tool-toggle is-active" : "tool-toggle"} title="N2YO Satellite tracker">
          <Satellite className="w-4 h-4" /> SAT
        </button>
        <button onClick={() => setShowJammingPanel(!showJammingPanel)} className={showJammingPanel ? "tool-toggle is-active" : "tool-toggle"} title="GPS jamming monitor">
